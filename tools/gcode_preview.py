...
motion/config.py. It needs no hardware, no serial port, and no Stockfish.
    ...
    python tools/gcode_preview.py --move g1f3 --high-lift
...
_SCENARIOS = [
    ("standard pawn e2e4", dict(uci_move="e2e4")),
    ("knight g1f3 (high lift)", dict(uci_move="g1f3", high_lift=True)),
    ("capture d4e5", dict(uci_move="d4e5", is_capture=True)),
    ("castling kingside e1g1", dict(uci_move="e1g1", move_type="castling")),
    ("castling queenside e8c8", dict(uci_move="e8c8", move_type="castling")),
    ("en passant d5e6", dict(uci_move="d5e6", move_type="en_passant",
                             is_capture=True)),
    ("promotion a7a8q", dict(uci_move="a7a8q", move_type="promotion")),
    ("promotion+capture b2a1q (black)", dict(uci_move="b2a1q",
                                             move_type="promotion",
                                             is_capture=True)),
    ("corner to corner a1h8", dict(uci_move="a1h8")),
]
...
def main() -> int:
    ...
    print(f"Z:      grip {cfg.Z_GRIP}  low carry {cfg.Z_CARRY_LOW}  "
          f"high carry/safe {cfg.Z_SAFE}  board {cfg.Z_BOARD}")
    print(f"claw:   open A{cfg.CLAW_OPEN_A:.0f}  closed A{cfg.CLAW_CLOSED_A:.0f}")
